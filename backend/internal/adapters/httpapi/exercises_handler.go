...
func NewExercisesHandler(s *exercises.Service) *ExercisesHandler { return &ExercisesHandler{svc: s} }
...
func (h *ExercisesHandler) LinkBehaviorExercise(w http.ResponseWriter, r *http.Request) {
	...
	if err := json.NewDecoder(r.Body).Decode(&cmd); err != nil { writeError(w, 400, "invalid json"); return }
	if err := h.svc.LinkBehavior(r.Context(), cmd); err != nil { writeError(w, 400, err.Error()); return }
