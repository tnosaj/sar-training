...
func NewSkillsHandler(s *skills.Service) *SkillsHandler { return &SkillsHandler{svc: s} }
...
func (h *SkillsHandler) Create(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		...
		if err == common.ErrConflict { writeError(w, 409, "name exists"); return }
...
func (h *SkillsHandler) Update(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		...
		if err == common.ErrNotFound { writeError(w, 404, "not found"); return }
		writeError(w, 500, err.Error()); return
	}
	writeJSON(w, 200, res)
}
