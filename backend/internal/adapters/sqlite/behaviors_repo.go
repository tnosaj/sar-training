...
func NewBehaviorsRepo(db *sql.DB) *BehaviorsRepo { return &BehaviorsRepo{db: db} }
...
func (r *BehaviorsRepo) List(ctx context.Context, skillID *int64) ([]*behavior.Behavior, error) {
	...
	args := []any{}
	if skillID != nil { query += ` WHERE skill_id = ?`; args = append(args, *skillID) }
	...
	for rows.Next() {
		...
		var c,u string
		if err := rows.Scan(&b.ID, &b.SkillID, &b.Name, &b.Description, &c, &u); err != nil { return nil, err }
...
func (r *BehaviorsRepo) Get(ctx context.Context, id behavior.BehaviorID) (*behavior.Behavior, error) {
	...
	var b behavior.Behavior
	var c,u string
	if err := row.Scan(&b.ID, &b.SkillID, &b.Name, &b.Description, &c, &u); err != nil { return nil, err }
	b.CreatedAt, _ = time.Parse(time.RFC3339, c)
	b.UpdatedAt, _ = time.Parse(time.RFC3339, u)
	return &b, nil
}
